...
def to_credential(  # pylint: disable=R0913
    ...
    """ Mapper """
    vertex_project = integration_data["settings"]["project"]
    vertex_location = integration_data["settings"]["zone"]
    vertex_credentials = vault_client.unsecret(integration_data["settings"]["service_account_info"])
    #
    credential_values = {
...
def to_test(  # pylint: disable=R0913
    ...
    if integration_data["settings"]["models"]:
        ...
        #
        model_values["model"] = f"vertex_ai/{model_name}"
    #
    return {
        "litellm_params": {
            "custom_llm_provider": "vertex_ai",
